      ...
      <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
        ${menuItems
          ...
          .map((item) => {
            ...
            return `<a href="${item.link}" class="${baseClasses} ${activeClasses}">${styledIcon}${item.name}</a>`;
          })
          .join('')}
      </nav>
...
globalThis.initLayout = function initLayout(pageId, pageTitle) {
...
async function loadNotifications() {
  try {
    notifications = await apiFetch('/api/invoice/notifications', { headers: authHeaders() }) || [];
    renderNotifications();
  } catch (error_) {
    console.error('Failed to load notifications', error_);
  }
}
      ...
      return `
        <div class="px-4 py-3 hover:bg-slate-50 border-b border-slate-100 flex gap-3 items-start transition-colors cursor-pointer">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm text-slate-800 font-medium">${notification.message}</p>
            ${notification.time ? `<p class="text-xs text-slate-500 mt-0.5">${fmtDate(notification.time)}</p>` : ''}
          </div>
        </div>
      `;
    })
...
globalThis.toggleNotifications = function toggleNotifications() {
  ...
  if (dropdown) {
    dropdown.classList.toggle('hidden', !isNotificationOpen);
  }
};
